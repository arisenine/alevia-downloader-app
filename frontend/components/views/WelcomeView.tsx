...
import { Download, Zap, Shield, Star, TrendingUp, Users, Clock } from 'lucide-react';
...
const WelcomeView = React.memo(() => {
  ...
  return (
    <div className="space-y-6">
      {/* Hero Section */}
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6 lg:p-8 text-center">
        <div className="w-16 h-16 mx-auto mb-6 bg-blue-600 dark:bg-blue-500 rounded-xl flex items-center justify-center shadow-lg">
          <Download className="w-8 h-8 text-white" />
        </div>
        
        <h2 className="text-3xl lg:text-4xl font-bold mb-4 text-gray-900 dark:text-white">
          Selamat Datang di LevTools
        </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 max-w-2xl mx-auto mb-6">
          {features.map((feature, index) => (
            <div 
              key={index}
              className="group p-4 bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600 hover:shadow-md transition-all duration-200"
            >
        ...
        <div className="inline-flex items-center space-x-2 px-6 py-3 bg-blue-600 dark:bg-blue-500 text-white rounded-lg font-medium shadow-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
          {platforms.map((platform, index) => (
            <div 
              key={index}
              className="group text-center p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200"
            >
              <div className="w-10 h-10 mx-auto mb-2 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all duration-200">
                <i className={`${platform.icon} text-lg ${platform.color} group-hover:text-white`}></i>
              </div>
